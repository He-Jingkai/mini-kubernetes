...
func OutputGPUJOBResponse(li *clientv3.Client, response def.GPUJobResponse) {
	gpuJob := utils.GetGPUJobByName(li, response.JobName)
	...
	nameList := utils.GetPodReplicaListByPodName(li, gpuJob.PodName)
	fmt.Println("nameList:   ", nameList)
	if len(nameList) != 1 {
		panic("job's pod replica name list length!=1")
	}
	podInstanceID := nameList[0]
	podInstance := utils.GetPodInstanceByID(li, podInstanceID)
	podInstance.Status = def.SUCCEEDED
	utils.PersistPodInstance(podInstance, li)
	//更新PodInstanceIDList
