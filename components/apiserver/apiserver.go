...
import (
	...
	"mini-kubernetes/components/apiserver/delete_api"
	"mini-kubernetes/components/apiserver/function_api"
	"mini-kubernetes/components/apiserver/get_api"
	"mini-kubernetes/components/apiserver/gpu_job_api"
	"mini-kubernetes/components/apiserver/heartbeat_api"
	"mini-kubernetes/components/apiserver/register_api"
	"mini-kubernetes/components/coredns/coredns_utils"
...
func main() {
	...
	Start(utils.GetLocalIP().String(), fmt.Sprintf("%d", def.MasterPort), cli)
}
...
func Start(masterIp string, port string, client *clientv3.Client) {
	...
	nodeIDList := utils.GetNodeList(client)
...
func handleCreateFunction(c echo.Context) error {
	...
	function.URL = fmt.Sprintf("http://%s:%d/function/%s", utils.GetLocalIP().String(), def.ActiverPort, function.Name)
...
func handleCreateStateMachine(c echo.Context) error {
	...
	stateMachine.URL = fmt.Sprintf("http://%s:%d/state_machine/%s", utils.GetLocalIP().String(), def.ActiverPort, stateMachine.Name)
			...
			for _, w := range wc.Events {
				if w.Type == clientv3.EventTypePut {
					...
					if change.IP != "" {
						...
						utils.PersistService(cli, *service)
...
func CheckNode() {
	for nodeID, time_ := range HeartBeatMap {
		if time.Now().Sub(time_).Minutes() > 10 {
			node := utils.GetNodeByID(cli, nodeID)
			node.Status = def.NotReady
			utils.PersistNode(cli, node)
		}
	}
}
