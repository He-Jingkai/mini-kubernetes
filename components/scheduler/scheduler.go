...
func SchedulerMetaInit() {
	nodeIDList := utils.GetAllNodesID(schedulerMeta.EtcdClient)
	for _, nodeID := range nodeIDList {
		nodeInfo, replicasOnNode := GetInfoOfANode(schedulerMeta.EtcdClient, nodeID)
	...
	currentAllReplicas := utils.GetAllPodInstancesID(schedulerMeta.EtcdClient)
...
func HandlePodInstanceChange(news []string) {
	...
	fmt.Println(news)
	addeds, deleteds := utils.DifferTwoStringList(schedulerMeta.ScheduledPodInstancesName, news)
		...
		for _, node := range schedulerMeta.Nodes {
			for _, replica := range node.PodInstanceList {
				if replica.InstanceName == deleted {
					found = true
					utils.DeletePodInstanceFromNode(schedulerMeta.EtcdClient, node.NodeID, deleted)
	...
	for _, added := range addeds {
		...
		if success {
			...
			utils.AddPodInstanceToNode(schedulerMeta.EtcdClient, nodeID, podInstance)
...
func SchedulePodInstanceToNode(instanceID string) (success bool, nodeId int, podInstance *def.PodInstance) {
	{
		podInstanceTmp := utils.GetPodInstance(instanceID, schedulerMeta.EtcdClient)
	...
	notWithFilterResult := NotWithFilter(nodeIndexList, podInstance.NodeSelector.NotWith, schedulerMeta.Nodes)
	if len(notWithFilterResult) == 0 {
		success = false
		return
	}
	fmt.Println("[notWithFilterResult]", notWithFilterResult)
	withFilterResult := WithFilter(notWithFilterResult, podInstance.NodeSelector.With, schedulerMeta.Nodes)
	if len(withFilterResult) == 0 {
		success = false
		return
	}
	fmt.Println("[withFilterResult]", withFilterResult)
	CPU, memory := PodResourceRequest(podInstance)
	// TODO: memory的单位
	resourceFilterResult := ResourceFilter(schedulerMeta.EtcdClient,
		withFilterResult,
	...
	success = true
	choseNodeIndex := ChooseNode(schedulerMeta.EtcdClient, resourceFilterResult, schedulerMeta.Nodes)
