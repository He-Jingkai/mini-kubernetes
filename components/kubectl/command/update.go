...
func updateFunc(c *cli.Context) {
	...
	if c.Args()[0] == "soft" {
		...
		err, status := httpget.Put("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/update/soft/function").
		...
		response := ""
		body, _ := json.Marshal(request)
		err, status := httpget.Put("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/update/hard/function").
			ContentType("application/json").
			Body(bytes.NewReader(body)).
			GetString(&response).
			Execute()
