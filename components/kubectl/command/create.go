...
func createFunc(c *cli.Context) {
	...
	if last4 == "json" {
		...
		response := ""
		err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create/stateMachine").
		...
		case yaml.Pod_t:
			...
			err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create_pod").
				ContentType("application/json").
				Body(bytes.NewReader(body)).
				GetString(&response).
				Execute()
		...
		case yaml.ClusterIP_t:
			...
			response := ""
			body, _ := json.Marshal(request)
			err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create/clusterIPService").
				ContentType("application/json").
				Body(bytes.NewReader(body)).
				GetString(&response).
				Execute()
		...
		case yaml.Nodeport_t:
			...
			response := ""
			body, _ := json.Marshal(request)
			err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create/nodePortService").
				ContentType("application/json").
				Body(bytes.NewReader(body)).
				GetString(&response).
				Execute()
		...
		case yaml.Dns_t:
			...
			response := ""
			body, _ := json.Marshal(request)
			err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create/dns").
				ContentType("application/json").
				Body(bytes.NewReader(body)).
				GetString(&response).
				Execute()
		...
		case yaml.Deployment_t:
			...
			response := ""
			body, _ := json.Marshal(request)
			err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create/deployment").
				ContentType("application/json").
				Body(bytes.NewReader(body)).
				GetString(&response).
				Execute()
		...
		case yaml.Autoscaler_t:
			...
			response := ""
			body, _ := json.Marshal(request)
			err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create/autoscaler").
				ContentType("application/json").
				Body(bytes.NewReader(body)).
				GetString(&response).
				Execute()
		...
		case yaml.Gpujob_t:
			...
			response := ""
			body, _ := json.Marshal(request)
			err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create/gpuJob").
				ContentType("application/json").
				Body(bytes.NewReader(body)).
				GetString(&response).
				Execute()
		...
		case yaml.Activity_t:
			...
			response := ""
			body, _ := json.Marshal(request)
			err, status := httpget.Post("http://" + utils.GetLocalIP().String() + ":" + fmt.Sprintf("%d", def.MasterPort) + "/create/function").
				ContentType("application/json").
				Body(bytes.NewReader(body)).
				GetString(&response).
				Execute()
