...
func Initialize() {
	activerMeta.FunctionsNameList = utils.GetFunctionNameList(activerMeta.EtcdClient)
...
func HandleFunctionsNameListChange(functionNameList []string) {
	added, deleted := utils.DifferTwoStringList(activerMeta.FunctionsNameList, functionNameList)
...
func TriggerFunction(funcName string, parames map[string]string, body interface{}) (int, interface{}) {
	FlowCount(funcName)
	function := utils.GetFunctionByName(activerMeta.EtcdClient, funcName)
	fmt.Println("funcName:   ", funcName)
	fmt.Println("function:   ", function)
	podReplicaNameList := utils.GetPodReplicaIDListByPodName(activerMeta.EtcdClient, function.PodName)
	service := utils.GetServiceByName(activerMeta.EtcdClient, function.ServiceName)
	if len(podReplicaNameList) == 0 {
		utils.AddNPodInstance(function.PodName, 1)
		time.Sleep(30 * time.Second)
		//utils.StartService(function.ServiceName)
	}
...
func TriggerStateMachine(stateMachineName string, parames map[string]string, body interface{}) (int, interface{}) {
	stateMachine := utils.GetStateMachineByName(activerMeta.EtcdClient, stateMachineName)
	...
	for {
		...
		if type_ == "Task" {
			...
			for _, option := range choice.Choices {
				...
				part := GetPartOfJsonResponce(option.Variable, currentBody)
...
func ExpandAndShrink() {
	...
	for _, name := range activerMeta.FunctionsNameList {
		...
		AdjustReplicaNum2Target(activerMeta.EtcdClient, name, targetReplicaNum)
	}
}
